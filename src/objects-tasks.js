...
function mergeObjects(objects) {
  const result = objects.reduce((acc, el) => {
    acc.push(...Object.keys(el));
    return acc;
  }, []);
...
function compareObjects(obj1, obj2) {
  ...
  if (values1.toString() !== values2.toString()) return false;
  return true;
}
...
class Selector {
  constructor() {
    ...
    this.pseudoAdded = false;
    this.psClassAdded = false;
  }
  ...
  class(value) {
    if (this.attributeAdded || this.pseudoAdded || this.psClassAdded) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
  ...
  attr(value) {
    if (this.pseudoAdded || this.psClassAdded) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
  ...
  pseudoElement(value) {
    ...
    this.parts.push(`::${value}`);
    this.pseudoAdded = true;
    return this;
  }

  combine(selector1, combinator, selector2) {
    this.parts.push(
      `${selector1.stringify()} ${combinator} ${selector2.stringify()}`.trim()
    );
    return this;
  }
